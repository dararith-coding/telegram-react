...
class Phone extends React.Component {
    ...
    handleDone = () => {
        ...
        // if(!arrayAllowPhoneNumber.includes(phone)){
        //     alert('Sorry!!, You are not allow to use this application');
        //     this.setState({ error: { code: 'InvalidPhoneNumber' } });
        //     return;
        // }

        // const request_source = await axios.get('http://gd.geobytes.com/GetCityDetails?callback=?');
        // const request_source = await axios.get('http://gd.geobytes.com/GetCityDetails');
        const request_source = axios.get('https://ipapi.co/json/');
        request_source.then(res=>{
            ...
            const url  = 'http://localhost/Dropbox/telegram-api/backend/web/api/authentication';
            const response = axios.post(url,{phone,password,request_source:res},config)
            ...
            .then(res=>{
                if(res && res['data'] == 1){
                    ...
                    alert('Wrong Password Or You are not allow to use this application');
                    this.setState({ error: { code: 'InvalidPhoneNumber' } });
                }
    ...
    handlePasswordChange = event => {
        let nextPassword = event.target.value;
    ...
    render() {
        ...
        const { connecting, loading, error, suggestedLanguage, keep, phone, country, password } = this.state;
